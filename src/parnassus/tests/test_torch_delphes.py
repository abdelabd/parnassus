...
def process_hcal_pipeline(
    ...
    # HCal energy fractions from delphes_card_CMS_5_1.tcl
    energy_fractions = {
    ...
    # Build HCal eta/phi bins from delphes_card_CMS_5_1.tcl
    eta_phi_map = {}  # eta -> set of phi bin edges
    ...
    # Lists for output tensors (one per event)
    tower_tensors_list = []
    eflow_neutral_hadron_tensors_list = []
    eflow_track_tensors_list = []
    
    print("\nHCal SimpleCalorimeter: Processing...")
    
    # Process event by event, matching pap_tensors batches with ecal_eflow_tracks
    # ecal_eflow_tracks is a list of per-event tensors
    event_idx = 0
    for batch_particles in pap_tensors:
        event_numbers = torch.unique(batch_particles[:, CMAP["EVENT_NUMBER"]]).cpu().numpy()
        ...
        for event_num in tqdm(event_numbers):
            ...
            # Get ECal EFlow tracks for this event and ensure correct dtype (float64)
            if event_idx < len(ecal_eflow_tracks):
                tracks = ecal_eflow_tracks[event_idx].to(DEVICE).to(torch.float64)
            else:
                tracks = torch.zeros(0, particles.shape[1], dtype=torch.float64, device=DEVICE)
            
            # Run HCal forward pass
            result = hcal(particles, tracks)
            
            # Collect COLUMN_MAP format tensors for ROOT output
            tower_tensor = result['tower_tensor'].cpu()
            if tower_tensor.shape[0] > 0:
                tower_tensor[:, CMAP["EVENT_NUMBER"]] = float(event_num)
            tower_tensors_list.append(tower_tensor)
            
            # EFlowPhoton in HCal is actually EFlowNeutralHadron
            eflow_neutral_hadron_tensor = result['eflow_photon_tensor'].cpu()
            if eflow_neutral_hadron_tensor.shape[0] > 0:
                eflow_neutral_hadron_tensor[:, CMAP["EVENT_NUMBER"]] = float(event_num)
                # Set PID to 0 for neutral hadrons (not 22 like photons)
                eflow_neutral_hadron_tensor[:, CMAP["PID"]] = 0.0
            eflow_neutral_hadron_tensors_list.append(eflow_neutral_hadron_tensor)
            
            eflow_track_tensor = result['eflow_track_tensor'].cpu()
            if eflow_track_tensor.shape[0] > 0:
                eflow_track_tensor[:, CMAP["EVENT_NUMBER"]] = float(event_num)
            eflow_track_tensors_list.append(eflow_track_tensor)
    ...
    return {
        'tower_tensors': tower_tensors_list,
        'eflow_neutral_hadron_tensors': eflow_neutral_hadron_tensors_list,
        'eflow_track_tensors': eflow_track_tensors_list,
    }
...
def process_eflow_merger_pipeline(
    ...
    """
    Apply EFlowMerger to combine tracks, photons, and neutral hadrons.
    ...
    - OutputArray: eflow (written as ParticleFlowCandidate / EFlowObject)
    
    Note: In C++ Delphes, all objects are internally Candidate objects. The Merger
    just concatenates them, and the TreeWriter maps fields to ParticleFlowCandidate
    which is a superset of both Track and Tower fields. Missing fields are 0.
    
    In our tensor format, COLUMN_MAP already has all necessary fields for both
    track-like and tower-like objects, so we can just concatenate the tensors.
    
    Args:
        hcal_eflow_tracks: List of HCal EFlow track tensors (one per event)
        eflow_photons: List of EFlowPhoton tensors (one per event)
        eflow_neutral_hadrons: List of EFlowNeutralHadron tensors (one per event)
        
    Returns:
        List of merged EFlow object tensors (one per event)
    """
    
    # Initialize Merger module
    eflow_merger = Merger().to(DEVICE)
    
    merged_eflow_tensors = []
    
    print("\nEFlowMerger: Combining tracks, photons, and neutral hadrons...")
    
    # Process event by event
    for tracks, photons, neutral_hadrons in tqdm(
        zip(hcal_eflow_tracks, eflow_photons, eflow_neutral_hadrons), 
        total=len(hcal_eflow_tracks)
    ):
        # Merge all three inputs
        merged = eflow_merger([
            tracks.to(DEVICE), 
            photons.to(DEVICE), 
            neutral_hadrons.to(DEVICE)
        ])
        merged_eflow_tensors.append(merged.cpu())
    
    return merged_eflow_tensors
...
def validate_against_benchmark(
    ...
    # EFlowObject/ParticleFlowCandidate: superset of Track and Tower fields
    track_kinematic_vars = ['PID', 'Charge', 'P', 'PT', 'Eta', 'EtaOuter', 'Phi', 'T', 'X', 'Y', 'Z']
    tower_kinematic_vars = ['E', 'ET', 'Eta', 'Phi', 'T']
    eflow_kinematic_vars = ['PID', 'Charge', 'E', 'P', 'PT', 'Eta', 'Phi', 'T']  # Common subset
    ...
    branches = [
        ...
        EFlowMerger (merged tracks + photons + neutral hadrons)
...
def main(
    ...
    print("\n" + "="*80)
    print("STEP 7: Applying HCal SimpleCalorimeter")
    print("="*80)
    
    # HCal takes ECal EFlowTracks and ParticleAfterProp as input
    hcal_results = process_hcal_pipeline(
        pap_tensors, ecal_results['eflow_track_tensors']
    )
    ...
    # ========================================================================
    # STEP 9: Apply EFlowMerger (Merger of tracks + photons + neutral hadrons)
    # ========================================================================
    
    print("\n" + "="*80)
    print("STEP 9: Applying EFlowMerger (tracks + photons + neutral hadrons)")
    print("="*80)
    
    eflow_tensors = process_eflow_merger_pipeline(
        hcal_results['eflow_track_tensors'],
        ecal_results['eflow_photon_tensors'],
        hcal_results['eflow_neutral_hadron_tensors']
    )
    
    # Add EFlowObject branch to ROOT output
    # Note: In C++ Delphes, this is written as ParticleFlowCandidate class
    branches_torch_root.update({
        'EFlowObject': tensor_to_root_dict(eflow_tensors, 'EFlowObject', expected_event_nums),
    })
    
    print("\n✓ EFlowMerger complete")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Benchmark ROOT file from C++ Delphes for validation (CMS_6_1 card with ECal, HCal, Calorimeter, and EFlowMerger)"
    )
    parser.add_argument(

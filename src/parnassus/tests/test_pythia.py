...
def compare_particles(
    ...
    if particle_c.pid != particle_py.pid:
        return False, f"Particle {particle_idx}: PDG ID mismatch: {particle_c.pid} vs {particle_py.pid}"
    
    # Check status
    if particle_c.status != particle_py.status:
        return False, f"Particle {particle_idx}: Status mismatch: {particle_c.status} vs {particle_py.status}"
    ...
    if not math.isclose(particle_c.generated_mass, particle_py.generated_mass, rel_tol=rel_tol, abs_tol=abs_tol):
        return False, f"Particle {particle_idx}: Mass mismatch: {particle_c.generated_mass} vs {particle_py.generated_mass}"
...
def compare_vertices(
    ...
    if n_in_c != n_in_py:
        return False, f"Vertex {vertex_idx}: Number of incoming particles mismatch: {n_in_c} vs {n_in_py}"
    
    # Check number of outgoing particles
    n_out_c = len(v_c.particles_out)
    n_out_py = len(v_py.particles_out)
    if n_out_c != n_out_py:
        return False, f"Vertex {vertex_idx}: Number of outgoing particles mismatch: {n_out_c} vs {n_out_py}"
...
def compare_events(
    ...
    message : str
        Description of first mismatch found (empty if match=True)
    """
    ...
    if check_event_number:
        if evt_c.event_number != evt_py.event_number:
            event_match = False
            mismatch_messages.append(f"Event number mismatch: {evt_c.event_number} vs {evt_py.event_number}")

    ####################### Check number of particles #######################
    n_particles_mismatch = n_particles_c - n_particles_py
    if n_particles_mismatch < 0:
        event_match = False
        mismatch_messages.append("Python event has more particles than C++ event, which should not happen.")
    
    ####################### Check number of vertices #######################
    n_vertices_mismatch = n_vertices_c - n_vertices_py
    if n_vertices_mismatch < 0:
        event_match = False
        mismatch_messages.append("Python event has more vertices than C++ event, which should not happen.")
    
    ####################### Check that particles match #######################
    if check_particle_ordering:
        # Direct order comparison
        for i, (particle_c, particle_py) in enumerate(zip(evt_c.particles, evt_py.particles)):
        ...
        for i, particle_c in enumerate(particles_c_list):
            ...
            for j, particle_py in enumerate(particles_py_list):
                ...
                particle_match, _ = compare_particles(particle_c, particle_py, i, rel_tol, abs_tol, ignore_id=True)
                if particle_match:
            ...
            if not found_particle_match:
                ...
                # Skip detached or non-stable particles (pyhepmc bindings don't allow us to add these on Python side; but not relevant to physics)
                if exclude_detached:
                    if particle_c.status == 42:
                ...
                event_match = False
                mismatch_messages.append(f"Particle {i} (pid={particle_c.pid}, status={particle_c.status}, p={particle_c.momentum}) has no match in python event")
    ...
    for i, v_c in enumerate(vertices_c_list):
        ...
        if not found_vertex_match:
            # Skip detached or non-stable particles (pyhepmc bindings don't allow us to add these on Python side; but not relevant to physics)
            if exclude_detached:
                if len(v_c.particles_in)==1 and v_c.particles_in[0].status==42:
                    n_vertices_mismatch -= 1
                    continue  
            event_match = False
            mismatch_messages.append(f"Vertex {i} with {len(v_c.particles_in)} in, {len(v_c.particles_out)} out has no match in python event")
    ...
    if n_particles_mismatch != 0:
        event_match = False
        mismatch_messages.append(f"Particles mismatch: n_particles_mismatch = {n_particles_mismatch}")
...
def compare_hepmc_files(
    ...
    with pyhepmc.open(file_c, "r") as f_c, pyhepmc.open(file_py, "r") as f_py:
        ...
        for evt_c, evt_py in zip(f_c, f_py):
...
def test_hepmc3_generator(): # Tests HepMC3Generator end-to-end: top-level configuration, parallelization, and merging of hepmc files.
    ...
    with tempfile.TemporaryDirectory(delete=False) as log_dir, tempfile.TemporaryDirectory(delete=False) as output_dir:
        ...
        fpath_merged = generator.generate(
            n_events=N_EVENTS, max_workers=N_JOBS, debug=False
        )
        print(f"Wrote to file {fpath_merged}")
        with pyhepmc.open(fpath_merged, "r") as f_merged:
            n_events_merged = sum(1 for _ in f_merged)
            assert(n_events_merged == N_EVENTS), f"HepMC3Generator: Merged file has {n_events_merged} events, expected {N_EVENTS}"

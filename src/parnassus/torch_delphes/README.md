...
| `--mode` | `fullsim` \| `delphes` | `fullsim` | Target flavour. `fullsim` applies the CMS acceptance cuts (`--truth-pt-cut 0.25`, `--reco-pt-cut 1.0`, `--eta-cut 2.7`, `<= 0` disables each) and the per-event `n_truth_chad` charged-hadron truncation (`--no-chad-truncation` turns it off). `delphes` turns **all** of them off and ignores those four flags — diff-Delphes has to reproduce Delphes as-is. Recorded in the history metadata (`mode` + resolved cuts), so Step 3 replays the same selection with no extra flags. |
...
| `--mode`, `--truth-pt-cut`, `--reco-pt-cut`, `--eta-cut`, `--no-chad-truncation` | | same as Step 2 | Acceptance selection, resolved once and shared by every trial (`--mode delphes` = no cuts, no truncation). |

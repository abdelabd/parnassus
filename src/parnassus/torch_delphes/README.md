...
| `--mode` | `fullsim` \| `delphes` | `fullsim` | Target flavour. `fullsim` applies the CMS acceptance cuts (`--truth-pt-cut 0.25`, `--reco-pt-cut 1.0`, `--eta-cut 2.7`, `<= 0` disables each), the per-event `n_truth_chad` charged-hadron truncation (`--no-chad-truncation` turns it off) and the supercluster-scale photon merger (`--photon-merge-radius 0.045`). `delphes` turns **all** of them off — no cuts, no truncation, photon merger OFF — and ignores those flags: diff-Delphes has to reproduce Delphes as-is. Recorded in the history metadata (`mode` + resolved cuts + radius), so Step 3 replays the same selection with no extra flags. |
...
| `--mode`, `--truth-pt-cut`, `--reco-pt-cut`, `--eta-cut`, `--no-chad-truncation` | | same as Step 2 | Acceptance selection, resolved once and shared by every trial. `--mode delphes` = no cuts, no truncation, **and the photon merger forced OFF**: `search.photon_merge_radius` is validated but not searched (one TPE dimension fewer). One study = one mode — use a distinct `--study-name` per mode (the search refuses to resume a study run in the other mode). |

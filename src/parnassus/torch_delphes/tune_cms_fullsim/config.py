...
OBSERVABLES: list[str] = [
    ...
    "n_truth_chad",
    # BCE efficiency-loss survival labels (per truth row, ragged, row-aligned with
    # truth_particles; 1.0/0.0 = Hungarian truth<->reco matched or not, see
    # data._build_survival_labels). Target-side only, non-plottable.
    "bce_x",
]
...
# tracking count terms (labels come from the in-memory Hungarian matcher at load
# time). The calo count terms are unaffected either way. Default: "bce" in --mode
